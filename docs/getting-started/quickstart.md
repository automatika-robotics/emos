...
The quickstart above used `image_raw` as the camera topic. On a real robot, you'll need to:

1. **Match the topic name** to your camera driver's output. Check with `ros2 topic list` and update the recipe accordingly.
2. **Start the camera driver** before running the recipe. See [Preparing Your Hardware](installation.md#preparing-your-hardware) for mode-specific instructions.
3. **Container mode**: install and start the driver inside the container (it has access to all USB devices). See [Preparing Your Hardware](installation.md#preparing-your-hardware).

### Pre-flight Checklist

Before running any recipe, verify these four things:

```bash
# 1. Model server running?
curl http://localhost:11434/api/tags    # For Ollama

# 2. Sensor drivers publishing?
ros2 topic list

# 3. Data flowing?
ros2 topic hz /image_raw               # Should show non-zero Hz

# 4. Topic names match recipe?
# Compare ros2 topic list output with Topic(name=...) in recipe.py
```
