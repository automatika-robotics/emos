...
| **Pixi**            | `pixi shell --manifest-path ~/.local/share/emos/pixi.toml`, then `source ~/.local/share/emos/install/setup.sh`, then `python3 recipe.py`.                                                    | Works -- the CLI activates the pixi env and sources the colcon overlay for you. |
