...
| **Pixi**            | Use `pixi shell` (or `pixi run python recipe.py`) from the EMOS repo directory; bare `python` will not have `agents` / `kompass` on `sys.path`.                                              | Works -- the CLI activates the pixi env for you.       |
